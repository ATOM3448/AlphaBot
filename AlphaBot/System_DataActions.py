...
# Получение всех процессов с завершением позже конкретный ход
def get_future_process(player, turn):
    db = get_db(player)
    cursor = db.cursor()
...
# Получить статус технологии
def get_tech(player, type):
    db = get_db(player)
    cursor = db.cursor()

    cursor.execute(f"""SELECT {type} FROM technologies""")
...
# Установка статуса технологии
def set_tech(player, type, value):
    db = get_db(player)
    cursor = db.cursor()   
 
    cursor.execute(f"""UPDATE technologies SET {type} = {value}""") 
