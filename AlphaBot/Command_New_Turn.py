...
# Проверить процессы
    ...
    for j in range(19):
            db.set_res(user, 'общий', new_turn, db.itemslocal[j+1], db.get_res(user, 'общий', last_turn, db.itemslocal[j+1]) + db.get_income(user, 'общий', last_turn, db.itemslocal[j+42]))
            db.set_income(user, 'общий', new_turn, db.itemslocal[j+42], db.get_income(user, 'общий', last_turn, db.itemslocal[j+42]))
    ...
    for i in complite_process:
        if ((i[0] >= 20) and (i[0] <= 41)):
            db.set_ship(user, 'общий', new_turn, db.itemslocal[i[0]+1], db.get_ship(user, 'общий', last_turn, db.itemslocal[j+20]) + i[2])
