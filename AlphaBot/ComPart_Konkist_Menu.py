...
async def getShipMenu(ctx, bot):
    ...
    if((type == 3) and (not(check_slot(user, 1, count) or check_slot(user, 4, count)))):
        await ctx.send('Недостаточно производственных мощностей')
        return
    if((type == 6) and not(check_slot(user, 2, count-10) or check_slot(user, 4, count))):
        await ctx.send('Недостаточно производственных мощностей')
        return
    if((type == 10) and not(check_slot(user, 3, count-10) or check_slot(user, 4, count))):
        await ctx.send('Недостаточно производственных мощностей')
        return
...
async def getMyInv(ctx):
    user = ctx.message.author.id

    last_turn = db.get_turn(user, 'общий')

    embed1 = discord.Embed(title = 'Склад', description=f'Ход: {last_turn}\nВ наличии | Добыча', colour=0x008b)

    list_res = db.get_all_res_nn(user, 'общий', last_turn)
    list_income = db.get_all_income_nn(user, 'общий', last_turn)

    embed1.add_field(name = f"{res['zelezo']}", value = f"{list_res[0]} | {list_income[0]}", inline=True)
    embed1.add_field(name = f"{res['zest']}", value = f"{list_res[1]} | {list_income[1]}", inline=True)
    embed1.add_field(name = f"{res['titan']}", value = f"{list_res[2]} | {list_income[2]}", inline=True)
    embed1.add_field(name = f"{res['volst']}", value = f"{list_res[3]} | {list_income[3]}", inline=True)
    embed1.add_field(name = f"{res['gugol']}", value = f"{list_res[4]} | {list_income[4]}", inline=True)
    embed1.add_field(name = f"{res['gas']}", value = f"{list_res[5]} | {list_income[5]}", inline=True)
    embed1.add_field(name = f"{res['energy']}", value = f"{list_res[6]} | {list_income[6]}", inline=True)
    embed1.add_field(name = f"{res['kamen1']}", value = f"{list_res[7]} | {list_income[7]}", inline=True)
    embed1.add_field(name = f"{res['kamen2']}", value = f"{list_res[8]} | {list_income[8]}", inline=True)
    embed1.add_field(name = f"{res['kamen3']}", value = f"{list_res[9]} | {list_income[9]}", inline=True)
    embed1.add_field(name = f"{res['kamen4']}", value = f"{list_res[10]} | {list_income[10]}", inline=True)
    embed1.add_field(name = f"{res['sverol']}", value = f"{list_res[11]} | {list_income[11]}", inline=True)
    embed1.add_field(name = f"{res['relik']}", value = f"{list_res[12]} | {list_income[12]}", inline=True)
    embed1.add_field(name = f"{res['moh']}", value = f"{list_res[15]} | {list_income[15]}", inline=True)
    embed1.add_field(name = f"{res['antivec']}", value = f"{list_res[17]} | {list_income[17]}", inline=True)

    await ctx.send(embed = embed1)

    embed1 = discord.Embed(title = 'Флот', description=f'Ход: {last_turn}\nВ наличии | В процессе', colour=0x008b)

    list_ship = db.get_all_ship_nn(user, 'общий', last_turn)

    process = db.get_future_process(user, last_turn)

    list_ship_process = []
    for i in range(22):
        list_ship_process.append(0)
    
    for j in range(len(process)):
        if(process[j][0] < 100):
            list_ship_process[process[j][0] - 20] = process[j][2]

    # Лёгкие
    embed1.add_field(name = "Мусорщик", value = f"{list_ship[0]} | {list_ship_process[0]}", inline=True)
    embed1.add_field(name = "Каменщик", value = f"{list_ship[1]} | {list_ship_process[1]}", inline=True)
    embed1.add_field(name = "Лёгкий Верфяник", value = f"{list_ship[2]} | {list_ship_process[2]}", inline=True)
    embed1.add_field(name = "Истребитель-Капля", value = f"{list_ship[3]} | {list_ship_process[3]}", inline=True)
    embed1.add_field(name = "Каравелла", value = f"{list_ship[4]} | {list_ship_process[4]}", inline=True)

    # Средние
    embed1.add_field(name = "Средний Верфяник", value = f"{list_ship[5]} | {list_ship_process[5]}", inline=True)
    embed1.add_field(name = "Галера", value = f"{list_ship[6]} | {list_ship_process[6]}", inline=True)
    embed1.add_field(name = "Фрегат", value = f"{list_ship[7]} | {list_ship_process[7]}", inline=True)
    embed1.add_field(name = "Линкор", value = f"{list_ship[8]} | {list_ship_process[8]}", inline=True)

    # Тяжёлые
    embed1.add_field(name = "Тяжёлый Верфяник", value = f"{list_ship[9]} | {list_ship_process[9]}", inline=True)
    embed1.add_field(name = "Погибель", value = f"{list_ship[10]} | {list_ship_process[10]}", inline=True)
    embed1.add_field(name = "Корсар", value = f"{list_ship[11]} | {list_ship_process[11]}", inline=True)
    embed1.add_field(name = "Бомбардир", value = f"{list_ship[12]} | {list_ship_process[12]}", inline=True)
    embed1.add_field(name = "Браконьер", value = f"{list_ship[13]} | {list_ship_process[13]}", inline=True)
    embed1.add_field(name = "Серая Шляпа", value = f"{list_ship[14]} | {list_ship_process[14]}", inline=True)
    embed1.add_field(name = "Красный Шарф", value = f"{list_ship[15]} | {list_ship_process[15]}", inline=True)
    embed1.add_field(name = "Чёрная перчатка", value = f"{list_ship[16]} | {list_ship_process[16]}", inline=True)
    embed1.add_field(name = "Синий Сапог", value = f"{list_ship[17]} | {list_ship_process[17]}", inline=True)

    # Вспомогательные
    embed1.add_field(name = "Станция", value = f"{list_ship[18]} | {list_ship_process[18]}", inline=True)
    embed1.add_field(name = "Научное судно", value = f"{list_ship[19]} | {list_ship_process[19]}", inline=True)
    embed1.add_field(name = "Вахтер", value = f"{list_ship[20]} | {list_ship_process[20]}", inline=True)
    embed1.add_field(name = "Солярист", value = f"{list_ship[21]} | {list_ship_process[21]}", inline=True)

    embed1.add_field(name = "ОТЦОВСКИЙ КОРАБЛЬ", value = f"{list_ship[22]}", inline=True)

    await ctx.send(embed = embed1)

async def getCraftMenu(ctx, bot):
    user = ctx.message.author.id

    last_turn = db.get_turn(user, 'общий')

    embed1 = discord.Embed(title = 'Доступные к созданию ресурсы', description='Для создания отправьте номер синтетика', colour=0x008b)

    embed1.add_field(name = f"1. {res['energy']}", value = f"Способы:\n1. {res['gugol']} 1 к 2\n2. {res['gas']} 1 к 3", inline=True)
    embed1.add_field(name = f"2. {res['sverol']}", value = f"Способы:\n1. 100{res['volst']} 300{res['gas']} 1000{res['moh']}\n2. 20000{res['mikrobot']}", inline=True)
    embed1.add_field(name = f"3. {res['antivec']}", value = f"5000{res['energy']} 1{res['sverol']}", inline=True)
    embed1.add_field(name = f"4. {res['relik']}", value = f"25{res['kamen1']} 25{res['kamen2']} 25{res['kamen3']} 25{res['kamen4']}", inline=True)
    ...
    check = lambda m: m.author == ctx.author and m.channel == ctx.channel
    try:
        type = await bot.wait_for('message', check=check, timeout=20)
    except asyncio.TimeoutError:
        await ctx.send('Команда создания отменена. Время вышло')
        return
    ...
    if(not((type >= 1) and (type <= 4))):
        await ctx.send('Не верный тип')
        return

    # Если тип равен 1 или 2 - запрашиваем способ
    if((type == 1) or (type == 2)):
        await ctx.send('Отправьте способ создания')
        try:
            way = await bot.wait_for('message', check=check, timeout=20)
        except asyncio.TimeoutError:
            await ctx.send('Команда создания отменена. Время вышло')
            return
        ...
        # Преобразование типа, для дальнейшей работы
        try:
            way = int(way.clean_content)
        except ValueError:
            await ctx.send('Убедитесь, что вводите число')
            return

        # Проверяем корректность ввода
        if(not((way >= 1) and (way <= 2))):
            await ctx.send('Не верный способ')
            return
    
    await ctx.send('Отправьте количество повторений')
    
    # Ожидаем ввод
    try:
        count = await bot.wait_for('message', check=check, timeout=20)
    except asyncio.TimeoutError:
        await ctx.send('Команда создания отменена. Время вышло')
        return
    ...
    if(type == 1):
        if(way == 1):
            list_res.append(db.itemslocal[5])
        else:
            list_res.append(db.itemslocal[6])
        list_price.append(1)
    ...
    if(type == 2):
        if(way == 1):
            list_res.append(db.itemslocal[4])
            list_res.append(db.itemslocal[6])
            list_res.append(db.itemslocal[16])

            list_price.append(100)
            list_price.append(300)
            list_price.append(1000)
        else:
            list_res.append(db.itemslocal[15])
    ...
    if(type == 4):
        list_res.append(db.itemslocal[8])
        list_res.append(db.itemslocal[9])
        list_res.append(db.itemslocal[10])
        list_res.append(db.itemslocal[11])
    ...
    if(type == 1):
        if(way == 1):
            db.set_res(user, 'общий', last_turn, db.itemslocal[7], db.get_res(user, 'общий', last_turn, db.itemslocal[7]) + count*2)
        else:
            db.set_res(user, 'общий', last_turn, db.itemslocal[7], db.get_res(user, 'общий', last_turn, db.itemslocal[7]) + count*3)

    if(type == 2):
        db.set_res(user, 'общий', last_turn, db.itemslocal[12], db.get_res(user, 'общий', last_turn, db.itemslocal[12]) + count)

    if(type == 3):
        db.set_res(user, 'общий', last_turn, db.itemslocal[18], db.get_res(user, 'общий', last_turn, db.itemslocal[18]) + count)

    if(type == 4):
        db.set_res(user, 'общий', last_turn, db.itemslocal[13], db.get_res(user, 'общий', last_turn, db.itemslocal[13]) + count)

    # Сообщение о успехе
    await ctx.send("Всё прошло успешно")

async def getMenu(ctx, bot):
    user = ctx.message.author.id

    last_turn = db.get_turn(user, 'общий')

    embed1 = discord.Embed(title = 'Меню управления', description=f'Прожмите реакцию для перехода в меню\nТекущий ход: {last_turn}', colour=0x008b)

    embed1.add_field(name = "1️⃣", value = f"Ваш склад и флот", inline=True)
    embed1.add_field(name = "2️⃣", value = f"Распределить мусорщики", inline=True)
    embed1.add_field(name = "3️⃣", value = f"Распределить каменьщики", inline=True)
    embed1.add_field(name = "4️⃣", value = f"Меню технологий", inline=True)
    embed1.add_field(name = "5️⃣", value = f"Создание синтетиков", inline=True)
    embed1.add_field(name = "6️⃣", value = f"Постройка кораблей", inline=True)

    panel = await ctx.send(embed = embed1)

    await panel.add_reaction('1️⃣')
    await panel.add_reaction('2️⃣')
    await panel.add_reaction('3️⃣')
    await panel.add_reaction('4️⃣')
    await panel.add_reaction('5️⃣')
    await panel.add_reaction('6️⃣')
    ...
    try:
        ...
        await ctx.send("Меню удалено. Время вышло")
        panel.delete()
    ...
    if(str(reaction.emoji) == '2️⃣'):
        await panel.delete()
        await getManageGarbMenu(ctx, bot)
    if(str(reaction.emoji) == '3️⃣'):
        await panel.delete()
        await getManageRockMenu(ctx, bot)
    if(str(reaction.emoji) == '4️⃣'):
        await panel.delete()
        await getTechMenu(ctx, bot)
    if(str(reaction.emoji) == '5️⃣'):
        await panel.delete()
        await getCraftMenu(ctx, bot)
    if(str(reaction.emoji) == '6️⃣'):
        await panel.delete()
        await getShipMenu(ctx, bot)